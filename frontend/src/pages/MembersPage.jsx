...
export function MembersPage() {
	...
	const handleDeleteMember = (id) => {
		setDeletingId(id);
		try {
			dispatch(deleteMember(id)); // your API call
	...
	return (
		<div>
			<div className="mb-7 flex flex-col sm:flex-row sm:items-start justify-between gap-4">
				<div>
					...
					<div className="text-[13px] text-text-muted">
						{sourceMembers.length} total members registered
					</div>
				</div>
				<button
			...
			<div className="overflow-hidden rounded-2xl border border-border-main bg-card-bg h-4/5">
				<div className="overflow-x-auto h-full">
					{isFetching ? (
						<PageLoader label="Loading membership data..." />
					) : (
			...
			{showModal ? (
				<MemberModal member={selectedMember} onClose={handleModal} />
			) : (
				""
			)}
		</div>
